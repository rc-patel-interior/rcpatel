...
import { ArrowRight, Mail, MapPin, Phone, Instagram, Menu, X, CheckCircle } from "lucide-react";
...
const WHATSAPP_NUMBER = "918976637493";
...
function ContactSection() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.name || !form.phone || !form.projectType || !form.message) return;

    const lines = [
      `*New Enquiry – RC Patel Interior*`,
      ``,
      `*Name:* ${form.name}`,
      `*Phone:* ${form.phone}`,
          ...
          <div className="bg-secondary/30 p-6 sm:p-8 md:p-10 border border-border/50">
            {submitted ? (
              <div className="flex flex-col items-center justify-center h-full text-center py-12 gap-4">
                ...
                <h3 className="text-2xl font-light">Opening WhatsApp!</h3>
                <p className="text-muted-foreground text-sm max-w-xs">
                  Your enquiry is ready. Complete the chat on WhatsApp to send it directly to us.
                </p>
              ...
              <>
                ...
                <form className="space-y-5" onSubmit={handleSubmit}>
                  ...
                  <Button
                    type="submit"
                    className="w-full rounded-none bg-[#25D366] hover:bg-[#1ebe5d] text-white h-12 sm:h-14 uppercase tracking-widest text-xs sm:text-sm mt-2 flex items-center justify-center gap-2"
                  >
